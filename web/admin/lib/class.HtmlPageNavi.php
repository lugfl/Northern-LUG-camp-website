...
function htmlpage_link($pagename,$content='',$args = Array()) {
	global $PAGE;
	$ret = '';
	...
	if(isset($PAGE[$pagename]) ) {
		...
		$_args = $args;
		array_push($_args,'p='.$pagename);
		$ret = '<a href="./index.php?'.join('&',$_args).'">'.$content.'</a>';
	}
	return $ret;
}
...
function getNaviHtml($parent='root',$embed = false) {
	global $PAGE;
	...
	if(count($pages) > 0) {
		foreach($pages as $pname=>$pdata) {
			if(! htmlpage_is_hidden($pname) ) {
				$navilevel = $pdata['navilevel'] ? $pdata['navilevel'] : 0;
				
				//$l = '<a href="./index.php?p='.$pname.'">'.$pdata['name'].'</a>';
				$l = '<div class="navi'.$navilevel.'"><a href="./index.php?p='.$pname.'">'.$pdata['name'].'</a></div>';
				array_push($items,$l);
				if($embed) {
					// Subnavi in die Hauptnavi integriert
					$zs = $this->getNaviHtml($pname,1);
					array_push($items,$zs);
				}
			}
		}
	...
	if($parent == 'root' && auth_ok()) {
		$l = '<div class="navi1"><a href="./index.php?p=start&logout=1">Abmelden</a></div>';
		array_push($items,$l);
